...
const CreateSubevents = ({
  ...
  const handleTimeInput = (f, t, event) => {
    ...
    if (f === "start") {
      ...
      if (t === "hours") {
        if (input.length === 2) {
          startTimeMinuteRef.current.focus();
        }
        setStartTime({ ...startTime, hours: input });
      }
      if (t === "minutes") {
        setStartTime({ ...startTime, minutes: input });
      }
    } else {
      setErrors({ ...errors, endTime: false });
      if (t === "hours") {
        if (input.length === 2) {
          endTimeMinuteRef.current.focus();
        }
        setEndTime({ ...endTime, hours: input });
      }
      if (t === "minutes") {
        setEndTime({ ...endTime, minutes: input });
      }
    }
  };
  ...
  const validate = () => {
    const errors = {
      ...
      endTime:
        ...
        (endTime.hours.length === 2 && endTime.minutes.length !== 2),
  ...
  const validateAndSubmit = () => {
    if (validate()) {
      ...
      if (endTime.hours.length === 2 && endTime.minutes.length === 2) {
        event.end_time = `${endTime.hours}:${endTime.minutes}`;
      }
    ...
    <div className="flex-column align-center add-sub-event-wrapper">
      {existingEvents.sort(eventTimeComparator).map((e) => (
        <div
          key={e.id}
          className={`${
            addingNew ? "mobile-excluded" : ""
          } margin-top-medium flex-row existing-subevent`}
        >
          <label className="margin-left-medium sub-event-time-label">
          ...
          <FontAwesomeIcon
            className="sub-event-trash"
            icon={faTrashAlt}
            onClick={() => deleteCallback(e.id)}
          />
        </div>
      ))}
      ...
      {!addingNew && (
        <Button
          type="button"
          className="margin-top-large full-width"
      ...
      {!addingNew && (
        <Button
          type="button"
          primary
          className="margin-top-small margin-bottom-large full-width"
          onClick={() => {
            setAddingNew(true);
          }}
        >
          ...
          <div className="flex-column margin-top-large">
            ...
            <Form.Field error={errors.titleNO}>
              ...
              <Input
                value={titleNO}
                onChange={(e) => {
                  handleFieldInput(e.target.value, "title_NO");
                }}
                type="text"
                autoComplete="off"
              />
            ...
            <Form.Field error={errors.titleEN}>
              <label className="form-field-title margin-top-small">
                Engelsk tittel
              </label>
              <Input
                value={titleEN}
                onChange={(e) => {
                  handleFieldInput(e.target.value, "title_EN");
                }}
                type="text"
                autoComplete="off"
              />
            ...
            <Form.Field error={errors.address}>
              ...
              {suggestedAddress && (
                <label
                  ...
                  onClick={() => {
                    handleAddressInput(suggestedAddress);
                    setSuggestedAddress(undefined);
                  }}
                >
                  Bruk foreslått adresse: {suggestedAddress}
                </label>
              )}
              <Input
                value={address}
            ...
            <Button
              type="submit"
              primary
              className="margin-top-small margin-bottom-large"
